...
cat("Este chunk 3 só aparece na saída PDF.\n")
...
# Exibir a tabela
library(xtable)
print(xtable(tabela_bayes),
      include.rownames=FALSE,
      include.colnames=TRUE)
```

Para gerar uma tabela no formato padrão do IBGE, para exibir saída somente em `html`.

```{r}
#| label: "apenas-html"
#| format: html

cat("Este chunk 4 só aparece na saída HTML.\n")

library(webshot2)
# webshot2::install_phantomjs() não é necessário para webshot2
library(knitr)
library(kableExtra)

# Gerar tabela em HTML com formatação padrão IBGE
tabela_html <- kable(
  tabela_bayes,
  format  = "html",
  caption = "Tabela - Força da Evidência do Fator de Bayes",
  align = c("r", "l")
) %>%
  kable_styling(
...
# Exibir tabela HTML no RStudio Viewer
tabela_html

```

Agora a mesma tabela adequada para o formato pdf.

```{r, results='asis'}
#| label: "apenas-pdf-2"
#| format: pdf

cat("Este chunk 5 só aparece na saída PDF.\n")

library(knitr)
library(kableExtra)

# Gerar tabela em PDF com formatação padrão IBGE
# Para gerar PDF, é necessário estar em um documento RMarkdown (.Rmd)
...
Um outro critério ilustrativo.

```{r}

# Carregar pacotes necessários
# install.packages("dplyr") # Descomente se não tiver o pacote instalado
library(dplyr)

# Função para classificar a força da evidência baseada no Fator de Bayes
classificar_evidencia <- function(bf) {
  if (bf < 1) {
    return("Evidência a favor da hipótese nula")
  } else if (bf < 3) {
    ...
    return("Evidência extrema")
  }
}

# Exemplo de valores de Fator de Bayes
valores_bf <- c(1.0, 2.0, 3.0, 4.0, 9, 10, 19, 20, 29, 30, 99, 100, 149, 150, 151, 152, 200, 300, 400, 500)

# Gerar a tabela com BF e Força da Evidência
tabela_bayes <- data.frame(
  Fator_de_Bayes = valores_bf
) %>%
  mutate(Forca_da_Evidencia = sapply(Fator_de_Bayes, classificar_evidencia))
