...
teste_policia <- chisq.test(tabela_policia)
print(teste_policia)
...
residuos_pad <- teste_policia$stdres
...
Discutimos fatores de Bayes no capítulo anterior sobre estatística bayesiana - você pode se lembrar de que representa a razão da verossimilhança dos dados sob cada uma das duas hipóteses:
...
if(require(BayesFactor)) {
  bf_policia <- contingencyTableBF(tabela_policia, sampleType = "indepMulti")
  print(bf_policia)
} else {
...
Este teste mostra que há uma forte relação entre depressão e problemas de sono. Também podemos calcular o fator de Bayes para quantificar a força da evidência a favor da hipótese alternativa:
