...
## A lógica dos testes de significância {#sec-logica-TSHN}

A lógica dos testes estatísticos, assim como a dos intervalos de confiança, se baseia no [**questionamento**]{.underline} do [**que ocorreria se repetíssemos a amostra ou experimento muitas vezes**]{.underline}.

Agiremos novamente como se as "condições simples" listadas em "Condições simples para inferência sobre uma média", no Capítulo 16, fossem verdadeiras: temos uma AAS perfeita de uma população exatamente Normal com desvio-padrão $\sigma$ conhecido por nós. Eis um exemplo que analisaremos.
...
Cada provador atribui um escore ao refrigerante novamente após o armazenamento. Esse é [**um experimento de dados emparelhados**]{.underline}. Nossos dados são as diferenças (escore antes do armazenamento menos escore após o armazenamento) dos escores dos provadores. Quanto maior a diferença (diferença \> 0), maior será a perda de doçura.
...
## Estabelecimento de hipóteses {#sec-hipoteses-TSHN}
...
**As hipóteses devem expressar as expectativas ou suspeitas que temos [antes]{.underline} de vermos os dados.** É [**trapaça**]{.underline} olhar primeiro os dados e então estabelecer hipóteses que se ajustem ao que os dados mostram.
:::

## Valor P e significância estatística {#sec-valor-p-TSHP}

A ideia do estabelecimento de uma hipótese nula, contra a qual desejamos encontrar [**evidência**]{.underline}, parece estranha no início. Pode ser útil pensar em um julgamento criminal.

O acusado é "inocente até que se prove o contrário".

É exatamente assim que funcionam os testes estatísticos de significância, embora, em estatística, lidemos com [**evidência**]{.underline} fornecida por [**dados**]{.underline} e usemos a [**probabilidade**]{.underline} para dizer [**quão**]{.underline} forte ou [**significante**]{.underline} é a evidência.

A [**probabilidade**]{.underline} que mede a força da evidência contra a hipótese nula é chamada de [**valor P**]{.underline}.
...
Valores [**usualmente grandes**]{.underline} da [**estatística**]{.underline} mostram que [**os dados não são consistentes**]{.underline} com $H_0$.

A [**probabilidade**]{.underline}, calculada [**supondo**]{.underline} $H_0$ [**verdadeira**]{.underline}, de que a [**estatística de teste**]{.underline} assuma [***um valor tão ou mais extremo***]{.underline} do que o [**valor realmente observado**]{.underline} é chamada de [**valor P**]{.underline} do [***teste***]{.underline}.

**Quanto [menor]{.underline} o [valor P]{.underline}, [mais]{.underline} forte é a [evidência]{.underline} contra** $H_0$ fornecida pelos [**dados**]{.underline}.

[**Valores P pequenos**]{.underline} **são [evidência]{.underline} contra** $H_0$, pois ***afirmam*** ***que o [resultado observado]{.underline} tem [ocorrência improvável]{.underline}*** [***se***]{.underline} $H_0$ for [***verdadeira***]{.underline}.

[**Valores P grandes**]{.underline} [**nãofornecem evidência contra**]{.underline} $H_0$.
:::
...
Um valor tão grande quanto $\bar{x} = 0,3$ apareceria por acaso em 17% de todas as amostras, quando $H_0: \mu = 0$ fosse verdadeira. Assim, a observação de $\bar{x} = 0,3$ [**não**]{.underline} **é evidência** forte ***contra*** $H_0$.

Por outro lado, pode-se verificar que a probabilidade de que $\bar{x}$ seja 1,02 ou maior, quando de fato $\mu = 0$, é de apenas 0,0006. Ou seja, raramente observaríamos uma perda média de doçura de 1,02 ou maior se $H_0$ fosse verdadeira. Esse [**valor P pequeno**]{.underline} fornece forte [**evidência contra**]{.underline} $H_0$ e a favor da alternativa $H_a: \mu > 0$.
:::

A [**hipótese alternativa**]{.underline} estabelece a [**direção**]{.underline} que [**conta**]{.underline} como [**evidência**]{.underline} [**contra**]{.underline} $H_0$. No Exemplo 17.5, apenas valores grandes, positivos, contam, porque a alternativa é unilateral do lado mais alto. Se a alternativa for bilateral, ambas as direções contam.
...
**A conclusão do Exemplo 17.6 [não]{.underline} é que** $H_0$ seja verdadeira.
...
[***Testes de significância avaliam a evidência contra***]{.underline} $H_0$.
...
[**Apenas**]{.underline} **dados que são inconsistentes com** $H_0$ nos permitem fazer uma afirmativa positiva de que temos forte **evidência contra** $H_0$.
:::
...
Não há uma regra sobre [***quão pequeno um valor P deva ser***]{.underline} para que rejeitemos $H_0$; é ***uma questão de julgamento*** e depende das circunstâncias específicas.
...
[**Se o valor P é tão pequeno quanto**]{.underline} $\alpha$, [**ou**]{.underline} [***menor do que***]{.underline} $\alpha$, [**dizemos**]{.underline} que os [**dados**]{.underline} [***são***]{.underline} [**estatisticamente significantes**]{.underline} no [***nível***]{.underline} $\alpha$.

A quantidade $\alpha$ é chamada de **nível de significância**.

"[***Significante***]{.underline}", em linguagem estatística, não tem o sentido de "importante".

[**Significa**]{.underline} simplesmente **"[*improvável de acontecer apenas por acaso*]{.underline}"**.
...
Se **P = 0,01**, [***temos um resultado***]{.underline} que, no [***longo prazo***]{.underline}, [***aconteceria não mais do que uma vez em 100 amostras***]{.underline}, [***se***]{.underline} $H_0$ fosse [***verdadeira***]{.underline}.
...
É **boa prática [interpretar]{.underline} as descobertas de significância** **estatística [*no contexto do problema*]{.underline}** para o qual os dados foram coletados.
...
## Testes para uma média populacional {#sec-testes-media-TSHO}
...
1.  **AAS**: a [***condição mais importante***]{.underline} é que os 72 executivos na **amostra** sejam **uma AAS** da **população de todos os executivos** **na empresa**. Devemos conferir essa exigência questionando como os dados foram produzidos.
...
## Exercícios selecionados {#sec-exercicios-TSHO}
