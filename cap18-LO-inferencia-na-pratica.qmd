...
1.  **Amostragem**: Temos uma amostra aleatória simples (AAS) da população de interesse. Não há não resposta ou outra dificuldade prática. A população é grande em comparação com o tamanho da amostra (N \> 20 . n, para que 1 AAS~c~ $\sim$ 1 AAS~s~).
...
**Sociólogo**: Um sociólogo da mesma universidade usa os alunos da disciplina de Sociologia 101 para ***examinar as atitudes com relação a pessoas pobres e programas de combate à pobreza***. Os [**alunos**]{.underline}, como um grupo, ***são mais jovens do que a população adulta como um todo***. Mesmo entre pessoas jovens, os alunos como um grupo ***provêm de lares mais educados*** e ***mais prósperos***. O [**sociólogo não pode**]{.underline}, razoavelmente, [***agir como se esses alunos fossem uma amostra aleatória de qualquer população de interesse***]{.underline}.

::: callout-note
## De onde vieram os dados?

O requisito mais importante de qualquer procedimento de inferência é que os [***dados provenham de um processo ao qual se apliquem as leis de probabilidade***]{.underline}.

A [**inferência**]{.underline} é mais confiável quando os dados resultam de uma **amostra aleatória** ou de um **experimento comparativo aleatorizado**.
:::
...
**Ao usar [inferência estatística]{.underline}, você [*age como se*]{.underline} seus [*dados fossem uma amostra aleatória*]{.underline} ou provenientes de um experimento comparativo aleatorizado.**

[**Se**]{.underline} seus dados [**não**]{.underline} provêm de uma amostra aleatória ou de um experimento comparativo aleatorizado, [***suas conclusões podem ser questionadas***]{.underline}.
:::
...
-   **Problemas práticos**, como [***não resposta***]{.underline} em amostras ou [***desistências***]{.underline} em um experimento, podem [***prejudicar***]{.underline} a [**inferência**]{.underline}, mesmo em um estudo bem planejado.

-   **Métodos diferentes** são necessários para planejamentos diferentes. Os procedimentos z [**não**]{.underline} são corretos para planejamentos amostrais aleatórios mais complexos do que uma AAS.

-   **Não há remédio** para [**falhas fundamentais**]{.underline}, como [***resposta voluntária***]{.underline} ou [***experimentos não controlados***]{.underline}.
:::

### Qual é a Forma da Distribuição Populacional?

A maioria dos procedimentos de inferência estatística exige algumas [**condições**]{.underline} sobre a [***forma***]{.underline} da [***distribuição populacional***]{.underline}.

Muitos dos [**métodos mais básicos**]{.underline} de [**inferência**]{.underline} são *planejados* para [**populações Normais**]{.underline}.

::: callout-tip
## Teorema Central do Limite na Prática

Os procedimentos z e muitos outros procedimentos planejados para distribuições Normais se baseiam na [**Normalidade**]{.underline} **da [*distribuição da média amostral*]{.underline}** $\bar{x}$, e não na Normalidade das observações individuais.

O teorema do central limite ([**TCL**]{.underline}) nos diz que: - A distribuição amostral de $\bar{x}$ é mais Normal do que as observações individuais - A [***distribuição amostral***]{.underline} de $\bar{x}$ [**se torna**]{.underline} [***mais Normal à medida que o tamanho da amostra aumenta***]{.underline}.
:::
...
Há uma [**importante exceção**]{.underline} ao princípio de que a forma da população seja menos crítica do que a procedência dos dados.

**Valores atípicos podem distorcer os resultados da inferência.**
:::

Qualquer procedimento de inferência que se baseie em [**estatísticas amostrais**]{.underline}, como a [**média amostral**]{.underline} $\bar{x}$, que [***não são resistentes a valores atípicos***]{.underline}, pode ser [***fortemente influenciado***]{.underline} por algumas [***poucas observações extremas***]{.underline}.

### Diretrizes Práticas

1.  **Explore seus dados** antes de fazer inferência
2.  **Faça um diagrama** de ramo e folhas ou um histograma de seus dados
3.  **Verifique** se a [**forma**]{.underline} é razoavelmente Normal
4.  **Procure sempre por [valores atípicos]{.underline}** e tente corrigi-los ou justificar sua remoção
5.  **Considere [métodos alternativos]{.underline}** que não exijam a Normalidade quando apropriado

## 18.2 Cuidados com os Intervalos de Confiança {#sec-cuidados-intervalos-IC}
...
A margem de erro em um intervalo de confiança cobre apenas **erros de amostragem aleatória**. Dificuldades práticas, como [**subcobertura**]{.underline} e [**não resposta**]{.underline}, são, muitas vezes, mais sérias do que os erros de amostragem aleatória. **A margem de erro não leva em consideração essas dificuldades.**
:::
...
**Resposta**: Provavelmente [**não**]{.underline}, porque as pessoas frequentemente [***mentem***]{.underline} sobre seu peso, especialmente em pesquisas por telefone.

## 18.3 Cuidados com os Testes de Significância {#sec-cuidados-testes-TSHo}
...
Você deve ter notado que o [**valor P**]{.underline} para o [**testeunilateral**]{.underline} é [**metade**]{.underline} do [**valor P**]{.underline} para o [**teste bilateral**]{.underline} da [***mesma hipótese nula***]{.underline} e ***com base*** nos [**mesmos dados**]{.underline}.
...
[**Suponha**]{.underline} que as 20 hipóteses nulas (nenhuma associação) para 20 testes de significância sejam ***todas verdadeiras***.

Então, [**cada teste tem uma chance de 5% de ser significante no nível 5%**]{.underline}.

Como 5% são 1/20, [**esperamos**]{.underline} que [**cerca de 1**]{.underline}, [***entre 20 testes***]{.underline}, [**forneça**]{.underline}, [***apenas devido ao acaso***]{.underline}, [***um resultado significante***]{.underline}.
...
Um [***exemplo sutil de análises múltiplas***]{.underline} é o **viés de publicação**.
...
Para publicar suas descobertas, os pesquisadores devem demonstrar que a nova terapia é [***eficaz***]{.underline} no nível de significância de 0,05.

[***Um dos pesquisadores obtém resultados estatisticamente significantes***]{.underline}, mas os [***outros 19 não***]{.underline}.

O [**único pesquisador**]{.underline} que obteve resultados estatisticamente significantes [***publica***]{.underline} suas descobertas.

[***Nada ficamos sabendo sobre os 19 pesquisadores que deixaram de encontrar significância estatística***]{.underline}.

Isso também pode ser visto como [***um viés de não resposta***]{.underline} (NA) por parte dos 19 que deixaram de enviar seus estudos para publicação.

OU também [***um viés de resposta voluntária***]{.underline}, por parte do único que enviou sua pesquisa paea publicação.
...
## Exercícios de Aplicação {#sec-exercicios-Inferencia-pratica}
...
## Resumo do Capítulo {#sec-resumo-Inferencia-pratica}
...
A **qualidade dos dados** é mais importante do que a **sofisticação dos métodos**.

Métodos estatísticos são ferramentas poderosas, mas só são [**eficazes**]{.underline} quando aplicados a dados de boa qualidade, coletados de forma apropriada.
:::